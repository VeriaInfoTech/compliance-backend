...
The project includes a Docker setup with PHP 8.3 FPM, nginx, MariaDB, Redis, and phpMyAdmin.
...
- App: **http://localhost:8085** (override with `HTTP_PORT` in `.env`).
- phpMyAdmin: **http://localhost:8095** (override with `PMA_PORT` in `.env`). Log in with `MYSQL_DB_USER` / `MYSQL_DB_PASSWORD`.
- MariaDB on host: port **3307** (override with `MARIADB_HOST_PORT` in `.env`) for external DB clients.

**Optional:** copy `.env.example` to `.env` and set `MYSQL_DB_PASSWORD`, `MYSQL_DB_NAME`, etc.
...
# Sync permissions (requires DB)
docker compose exec app php bin/sync-permissions.php
```

The `digrc-network` requirement was removed; the stack now uses an internal `shahr-network` and includes MariaDB and Redis services.
