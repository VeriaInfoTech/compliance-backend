    ...
    private function canonizePermission(array $permissionConfig): array
    {
        foreach ($permissionConfig as $permissionSection => $permissionList) {
            foreach ($permissionList as $permissionSingleKey => $permissionSingle) {
                // Many modules use `permission` (singular); installer expects `permissions` + `page`.
                if (!empty($permissionSingle['permission'])) {
                    if (empty($permissionSingle['permissions'] ?? null)) {
                        $permissionSingle['permissions'] = $permissionSingle['permission'];
                    }
                    if (empty($permissionSingle['page'] ?? null)) {
                        $permissionSingle['page'] = $permissionSingle['permission'];
                    }
                }
