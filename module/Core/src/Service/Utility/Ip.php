...
class Ip implements ServiceInterface
{
    ...
    /** Default ranges when config is missing (e.g. ErrorHandler passes []). */
    private const DEFAULT_LOCAL_RANGES = ['127.', '::1'];
    private const DEFAULT_INTERNAL_RANGES = ['10.', '172.', '192.168.', 'fc00:', 'fd00:'];
    ...
    public function __construct(
        ...
        $this->config = array_merge([
            'local_ranges'    => self::DEFAULT_LOCAL_RANGES,
            'internal_ranges' => self::DEFAULT_INTERNAL_RANGES,
        ], $config);
    ...
    {
        $ranges = $this->config['local_ranges'] ?? self::DEFAULT_LOCAL_RANGES;
        if (!is_array($ranges)) {
            $ranges = self::DEFAULT_LOCAL_RANGES;
        }
    ...
    {
        $ranges = $this->config['internal_ranges'] ?? self::DEFAULT_INTERNAL_RANGES;
        if (!is_array($ranges)) {
            $ranges = self::DEFAULT_INTERNAL_RANGES;
        }
        foreach ($ranges as $range) {
            if (str_starts_with($ip, $range)) {
                return true;
