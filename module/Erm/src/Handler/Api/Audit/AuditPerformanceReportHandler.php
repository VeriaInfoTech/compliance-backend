...
class AuditPerformanceReportHandler implements RequestHandlerInterface
{
...
//        $params['user_id'] = $account['id'];
        ...
        foreach ($this->taskService->getAuditList($params, $account)['data']['list'] as $task) {
            $allTaskCount++;
            if (!empty($task['audit']) && ($task['audit'] != new stdClass())) {
                switch ($task['audit']['level']) {
                    case "todo":
        ...
        $taskList = $this->taskService->getAuditList($params, $account)['data']['list'];
