...
class AuditProgressHandler implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        if ($params['type'] == 'child') {
            ...
            $progress = $this->taskService->getAuditTaskProgress(['id' => $params['progress_id'], 'task_id' => $params['task_id']]);
            if (!empty($progress)) {
...
//                }
            ...
            $progress = $this->taskService->getAuditTaskProgress(['task_id' => $params['task_id'], 'parent_id' => 0]);
            if (!empty($progress)) {
                if ($progress['user_id'] != $params['user_id']) {
                    return new JsonResponse([
                        'result' => false,
                ...
                if (in_array($params['level'], ['approve', 'done'])) {
                    $childProgress = $this->taskService->getAuditTaskProgress(['level' => ['todo', 'doing', 'reject', 'unassign'], 'parent_id' => $progress['id']]);
        ...
        $data = $this->taskService->AuditProgressTask($params, $account);
        $result = [
            'result' => true,
            'data' => $data,
            'error' => new \stdClass()
