...
namespace Erm\Handler\Api\Maturity;
...
class MaturityDashboardHandler implements RequestHandlerInterface
{
...
//        $params['user_id'] = $account['id'];
        ...
        $domainTree = $this->taskService->getDomainTree(['status' => [1], 'type' => 'maturity'], []);
        ...
        $domainMaturity[0][$domainTree[0]['slug']] = $domainTree[0];
        $domainMaturity[0]['children'] = $domainTree;

        foreach ($domainMaturity as $key => $node) {

            foreach ($node['children'] as $nodeKey => $child) {
                $flag = isset($radar[$domainMaturity[$key][$nodeKey]['slug']]);
                $slug = $domainMaturity[$key]['children'][$nodeKey]['slug'];
                $title = $domainMaturity[$key]['children'][$nodeKey]['title'];
                $domainMaturity[$key]['children'][$nodeKey] = [];

                $domainMaturity[$key]['children'][$nodeKey]['slug'] = $slug;
                $domainMaturity[$key]['children'][$nodeKey]['title'] = $title;
                $domainMaturity[$key]['children'][$nodeKey]['0'] =
                    $flag ?
                        $radar[$domainMaturity[$key]['children'][$nodeKey]['slug']]['0'] ?? 0 :
                        0;
                $domainMaturity[$key]['children'][$nodeKey]['50'] =
                    $flag ?
                        $radar[$domainMaturity[$key]['children'][$nodeKey]['slug']]['50'] ?? 0 :
                        0;
                $domainMaturity[$key]['children'][$nodeKey]['100'] =
                    $flag ?
                        $radar[$domainMaturity[$key]['children'][$nodeKey]['slug']]['100'] ?? 0 :
                        0;
                $domainMaturity[$key]['children'][$nodeKey]['in_progress_count'] =
                    $flag ?
                        $radar[$domainMaturity[$key]['children'][$nodeKey]['slug']]['in_progress_count'] ?? 0 :
                        0;
                $domainMaturity[$key]['children'][$nodeKey]['total_count'] =
                    $flag ?
                        $radar[$domainMaturity[$key]['children'][$nodeKey]['slug']]['total_count'] ?? 0 :
                        0;
        ...
        $dashboard['radar'] = $domainMaturity;
