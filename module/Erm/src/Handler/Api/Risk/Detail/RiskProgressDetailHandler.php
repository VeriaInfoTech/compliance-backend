...
namespace Erm\Handler\Api\Risk\Detail;
...
class RiskProgressDetailHandler implements RequestHandlerInterface
{
    ...
    public function handle(ServerRequestInterface $request): ResponseInterface
    {
        ...
        $progress = $this->taskService->getRiskTaskProgress(['id' => $params['progress_id'], 'task_id' => $params['task_id'], 'parent_id' => 0]);
        if ( empty($progress))  {
            return new JsonResponse([
                'result' => false,
                'data' => new \stdClass(),
        ...
        $data = $this->taskService->getRiskTaskProgressDetail($params, $account);
        $result = [
            'result' => true,
            'data' => $data,
            'error' => new \stdClass()
