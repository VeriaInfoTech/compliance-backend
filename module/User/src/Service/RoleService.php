...
class RoleService implements ServiceInterface
{
    ...
    /**
     * Normalize roles to array of role name strings (request may send [{"name":"admin"},...] or ["admin"]).
     */
    private function normalizeRoleNames($roles): array
    {
        if (!is_array($roles)) {
            return [];
        }
        ...
        foreach ($roles as $role) {
            if (is_array($role) && isset($role['name']) && $role['name'] !== '' && $role['name'] !== null) {
                $names[] = (string) $role['name'];
            } elseif (is_string($role) && $role !== '') {
                $names[] = $role;
            } elseif (is_scalar($role) && $role !== '') {
                $names[] = (string) $role;
            }
        }
        return $names;
    ...
    {
        $roles       = $this->normalizeRoleNames($roles);
        $roleList    = $this->getRoleList($section);
